...
# allegro_sim imports
from allegro_sim.utils import load_human_data
from openteach.utils.network import ZMQCameraPublisher, ZMQCompressedImageTransmitter,ZMQKeypointPublisher,ZMQKeypointSubscriber, ZMQCameraSubscriber


from .base_policy import BasePolicy

class HumanTeleop(BasePolicy):
    def __init__(
        self,
        expert_demos,
        expert_id,
        host = '172.24.71.211', # These values are set for this computer so will not be taking them as parameters 
        ...
        **kwargs
        ...
        self.timestamp_subscriberr = ZMQKeypointPublisher(
                        host='172.24.71.211',
                        port=10008
                )
        ...
        allegro_bounds_path = get_path_in_package(
            '/home/aadhithya/Holo-Bot/openteach/components/operators/configs/allegro.yaml'
        )
    ...
    def act(self, obs, episode_step, **kwargs):
        ...
        #Test if there are any keypoints being published in the socket if not end the base policy . Prone to inconsistencies in publishing
        try:
