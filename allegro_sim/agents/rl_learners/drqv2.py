...
from allegro_sim.utils import TruncatedNormal, schedule
from allegro_sim.models import weight_init
...
class Actor(nn.Module):
    def __init__(self, repr_dim, action_shape, hidden_dim, offset_mask=None):
        ...
        print('repr_dim in Actor: {}'.format(repr_dim))
    ...
    def forward(self, obs, action, std):
        ...
        print('obs.shape in actor.forward: {}'.format(obs.shape))
...
class DRQv2(RLLearner):
    ...
    def update_critic(self, obs, action, base_next_action, reward, discount, next_obs, step, **kwargs):
        ...
        with torch.no_grad():
            ...
            offset_action[:,:-6] *= self.hand_offset_scale_factor # NOTE: There is something wrong here?
            offset_action[:,-6:] *= self.arm_offset_scale_factor 
    ...
    def update_actor(self, obs, base_action, step, **kwargs):
        ...
        action_offset[:,:-6] *= self.hand_offset_scale_factor
        action_offset[:,-6:] *= self.arm_offset_scale_factor
