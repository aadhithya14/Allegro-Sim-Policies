...
def dump_video_to_images(root: str, video_type: str ='rgb', view_num: int=0, dump_all=True) -> None:
    ...
    while success: 
        ...
        if (not dump_all and frame_id == desired_indices[desired_img_id][1]) or dump_all:
            #image=crop_transform(image,1)
            image=image[150:,450:-100]
...
def dump_fingertips(root):
    ...
    with h5py.File(allegro_states_path, 'r') as f:
        #joint_positions = f['joint_angles'][()] # For sim
...
def get_shortened_ts_ids(
    ...
    with h5py.File(allegro_commanded_joint_path, 'r') as f:
        allegro_action_timestamps = f['timestampss'][()]
...
def dump_data_indices(
    ...
    with h5py.File(allegro_commanded_joint_path, 'r') as f:
        ...
        allegro_commanded_positions = f['positions'][()]
    with open(image_metadata_path, 'rb') as f:
        image_metadata = pickle.load(f)
        image_timestamps = np.asarray(image_metadata['timestamps']) / 1000.

    print('image_timestamps: {}, allegro_timestamps: {}'.format(
        image_timestamps, allegro_timestamps
    ))
    ...
    if shorten_demos:
        ...
        allegro_commanded_positions = allegro_commanded_positions[:shortened_ids['allegro_action']]
        # image
    ...
    image_timestamp=image_timestamps[0]
    allegro_id = get_closest_id(allegro_id, image_timestamp, allegro_timestamps)
    image_id = get_closest_id(image_id, image_timestamp, image_timestamps)
    allegro_action_id = get_closest_id(allegro_action_id, image_timestamp, allegro_action_timestamps)
    ...
    while (True):
        if is_byol_image:
            ...
            if subsample_separately:
                ...
                if pos_allegro_id >= len(allegro_positions):
                    break
               
                metric_timestamp = allegro_timestamps[pos_allegro_id]  
    ...
    print('image_indices: {}, allegro_indices: {}'.format(
        image_indices, allegro_indices
    ))

    assert len(image_indices) == len(allegro_indices)
    # Save the indices for that root
...
def crop_transform(image, camera_view=0, image_size=480): # This is significant to the setup
    if camera_view == 0:
        ...
        return crop(image, 100,200, image_size,image_size)
