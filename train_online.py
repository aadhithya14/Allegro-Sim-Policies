...
class Workspace:
    ...
    def train_online(self):
        ...
        #pub_image= self.train_env.pub_rgb_images()
        ...
        while train_until_step(self.global_step): # We're going to behave as if we act and the observations and the representations are coming from the mock_demo but all the rest should be the same
            ...
            # print('observations[image_obs].len: {}, image_obs.shape: {}, self.global_step: {}'.format(
            #     len(observations['image_obs']), observations['image_obs'].shape, self.global_step
            # ))
